...
export default function EconomicsResultScreen() {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScreenHeader title={t('economics.results') || 'ROI Results'} onBack={() => (navigation as any).goBack()} />
          ...
          <Section title="Recommended Species" styles={styles}>
            {simulationData.recommendedSpecies.map((species: any, idx: number) => (
              <View key={idx} style={styles.listCard}>
                <Text style={styles.listCardTitle}>{species.speciesName || species.species || 'Species'}</Text>
                <Text style={styles.listCardMeta}>Compatibility {species.compatibilityScore || species.score || '-'}%</Text>
              </View>
            ))}
          </Section>
        ) : null}
